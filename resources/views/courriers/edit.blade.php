        ...
        <section class="courier-section">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div class="form-group"  x-show="type === 'arrive'">
                    ...
                    @php 
                        // Determine prefix based on user's email
                        $editPrefix = '';
                        if (auth()->check()) {
                    ...
                    <input 
                        ...
                        data-prefix="{{ $editPrefix }}"
                        oninput="ensurePrefix(this)"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-colors duration-200"
                        placeholder="Ex: cab\\1234">
                    @if($editPrefix)
                        <p class="text-xs text-gray-500 mt-1">Préfixe requis: <strong>{{ $editPrefix }}</strong>. Seuls des chiffres sont autorisés après le préfixe.</p>
                    @endif
                    @error('reference_bo')
                        <p class="mt-1 text-sm text-red-600" role="alert">{{ $message }}</p>
                    @enderror
